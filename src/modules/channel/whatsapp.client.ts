  ...
  async sendTextMessage(params: SendWhatsAppParams): Promise<string | null> {
    ...
    if (!response.ok) {
      ...
      const action = isTokenExpired
        ...
        : isRecipientNotAllowed
          ? `Agrega el número ${params.to} en Meta Developers > WhatsApp > API Setup > lista de números de prueba (modo desarrollo).`
      ...
      logger.error(
        ...
        isTokenExpired ? "WhatsApp token expired while sending message" : "Failed to send WhatsApp message"
      );
